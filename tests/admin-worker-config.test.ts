...
describe("shadow admin Worker topology", () => {
	it("routes admin APIs to a private service without exposing that service", async () => {
		...
		expect(publicConfig).toMatchObject({
			...
			services: [{ binding: "ADMIN_API", service: "toen-admin-canary" }],
		});
		expect(adminConfig).toMatchObject({
			name: "toen-admin-canary",
		...
		expect(
			...
			expect.arrayContaining([
				"ACCESS_TEAM_DOMAIN_STORE",
				"ACCESS_POLICY_AUD_STORE",
				"GITHUB_APP_PRIVATE_KEY_STORE",
			]),
		);
	});
});
